...
export default function RepairsPage() {
  ...
  async function loadData() {
    ...
    try {
      const [vehRes, catRes, logRes] = await Promise.all([
        supabase.from('vehicles').select('*').eq('company_id', companyId),
        supabase.from('repair_categories').select('*').eq('company_id', companyId),
        supabase.from('repair_logs').select('*').eq('company_id', companyId).order('created_at', { ascending: false }),
      ])

      if (vehRes.error) console.error('Ошибка загрузки техники:', vehRes.error)
      else setVehicles(vehRes.data || [])

      if (catRes.error) console.error('Ошибка загрузки категорий:', catRes.error)
      else setCategories(catRes.data || [])

      if (logRes.error) {
        console.error('Ошибка загрузки ремонтов:', logRes.error)
      } else {
        const logsWithData = (logRes.data || []).map(log => {
          const vehicle = (vehRes.data || []).find(v => v.license_plate === log.license_plate)
          const category = (catRes.data || []).find(c => c.id === log.category_id)
          return {
            ...log,
  ...
  const filteredLogs = logs.filter(log => {
    ...
    return (
      ...
      log.category_name?.toLowerCase().includes(searchLower)
    )
  })
  ...
  return (
    <div style={{ padding: '24px 90px', position: 'relative' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 700, color: '#0f172a' }}>Ремонты</h1>
          <p style={{ fontSize: 14, color: '#94a3b8' }}>
            {totalLogs} записей ремонтов • Общая стоимость: ₽{totalCost.toLocaleString()}
          </p>
        </div>
        <button
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ position: 'relative', maxWidth: 400 }}>
          ...
          <input
            type="text"
            placeholder="Поиск по названию, номеру или категории..."
            value={search}
