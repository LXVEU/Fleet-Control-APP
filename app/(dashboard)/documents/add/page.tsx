...
import { FileText, Loader2, CheckCircle, AlertCircle, ArrowLeft, Calendar, Link2, CalendarX } from 'lucide-react'
...
export default function DocumentAddPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    expiryDate: '',
    fileUrl: '',
    hasExpiry: true, // чекбокс - есть ли срок действия
  })
  ...
  async function loadData() {
    ...
    try {
      const [companyRes, vehRes] = await Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.documentName.trim()) {
        throw new Error('Введите наименование документа')
      }

      if (!formData.startDate) {
        throw new Error('Выберите дату начала')
      }

      // Если есть срок действия, но дата не выбрана
      if (formData.hasExpiry && !formData.expiryDate) {
        throw new Error('Выберите дату окончания')
      }
      ...
      // Если срок действия отключен, передаем null
      const expiryValue = formData.hasExpiry ? formData.expiryDate : null
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={{ 
                  ...
                  Наименование документа <span style={{ color: '#dc2626' }}>*</span>
                </label>
                <input
                  type="text"
                  placeholder="Например: Страховой полис, СТС, ПТС"
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={{ 
                  ...
                  Дата начала <span style={{ color: '#dc2626' }}>*</span>
                </label>
                <div style={{ position: 'relative' }}>
                  <Calendar size={18} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                  <input
                    type="date"
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={{
                  ...
                  />
                  <span>У документа есть срок действия</span>
                </label>
              </div>

              {/* Дата окончания - показываем только если есть срок */}
              {formData.hasExpiry && (
                <div style={{ marginBottom: 16 }}>
                  <label style={{ 
                    display: 'block', 
                    ...
                    Дата окончания <span style={{ color: '#dc2626' }}>*</span>
                  </label>
                  <div style={{ position: 'relative' }}>
                    <CalendarX size={18} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                    <input
                      type="date"
